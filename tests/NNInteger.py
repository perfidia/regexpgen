...
class Test(unittest.TestCase):
	...
	def __runTest1__(self, scale, setMin, setMax):
		...
		for i in xrange(-max * 2, -1): #, (max + 100)/100):
			self.assertFalse(re.match(regexp, str(i)), self.__getInfo__(str(i), regexp, format, min, max))
		for i in xrange(0, min - 1):
			self.assertFalse(re.match(regexp, str(i)), self.__getInfo__(str(i), regexp, format, min, max))
		for i in xrange(min, max):
			a = "0" + str(i); b = "00" + str(i); c = "000" + str(i); d = "0000" + str(i);
			self.assertTrue(re.match(regexp, str(i)), self.__getInfo__(i, regexp, format, min, max))
			self.assertTrue(re.match(regexp, a), self.__getInfo__(a, regexp, format, min, max))
			self.assertTrue(re.match(regexp, b), self.__getInfo__(b, regexp, format, min, max))
			self.assertTrue(re.match(regexp, c), self.__getInfo__(c, regexp, format, min, max))
			self.assertTrue(re.match(regexp, d), self.__getInfo__(d, regexp, format, min, max))
		if (setMax):
	...
	def __runTest2__(self, scale, setMin, setMax):
		...
		for i in xrange(min, max):
			a = "0" + str(i); b = "00" + str(i); c = "000" + str(i); d = "0000" + str(i);
			self.assertTrue(re.match(regexp, str(i)), self.__getInfo__(i, regexp, format, min, max))
			self.assertFalse(re.match(regexp, a), self.__getInfo__(a, regexp, format, min, max))
			self.assertFalse(re.match(regexp, b), self.__getInfo__(b, regexp, format, min, max))
			self.assertFalse(re.match(regexp, c), self.__getInfo__(c, regexp, format, min, max))
			self.assertFalse(re.match(regexp, d), self.__getInfo__(d, regexp, format, min, max))
		if (setMax):
			for i in xrange(max+1, max * 2):
				self.assertFalse(re.match(regexp, str(i)), self.__getInfo__(str(i), regexp, format, min, max))
